...
module.exports = (db) => {
  ...
  router.get("/:employee_id/tasks", (req, res) => {
    ...
    let query = `SELECT first_name as fName, name, description, due_date, due_date - CURRENT_DATE AS pending_days, zoom, tasks_employee.id, tasks.id, rating, completion FROM tasks_employee
  ...
  router.get("/:employee_id/tasks/:task_id", (req, res) => {
    ...
    let query = `SELECT name, description, image, content, link, due_date, tasks_employee.id, tasks.id, rating, url, zoom, completion FROM tasks_employee
    JOIN tasks ON task_id = tasks.id
    JOIN employees ON employee_id = employees.id
